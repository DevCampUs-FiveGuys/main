...
public interface AttendanceMapperInter {
    ...
    @Insert("insert into attendance (check_in, member_id) values (now(), 1)")
    public void insertCheckIn(String check_in, int member_id);

    // 입실 후 퇴실 버튼을 누르면 퇴실 (check_out) 시간을 DB 에 저장
    @Update("update attendance set check_out = now() where DATE(check_in) = DATE(now()) and member_id = 1")
    public void updateCheckOut(String check_out, int member_id);

    // 출석현황 페이지 전체 조회
    @Select("select * from attendance where member_id = 1")
    ...
    @Update("update attendance set hospital = hospital + 1, absent = absent - 1 where DATE(check_in) = DATE(now()) and member_id = 1 and #{confirm} = 1")
